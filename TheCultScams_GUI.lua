-- <The Cult Scams> Matte UI - v4.4 [MINT] TS: 10:25:42
...
frame:SetBackdropColor(0.1, 0.25, 0.2, 0.95); frame:Hide()
...
local card = CreateFrame("Frame", "TheCultScams_RecipeCard", frame); card:SetWidth(280); card:SetHeight(180); card:SetPoint("CENTER", 0, 20); card:SetBackdrop({bgFile="Interface\\Tooltips\\UI-Tooltip-Background", edgeFile="Interface\\DialogFrame\\UI-DialogBox-Border", edgeSize=12}); card:SetBackdropColor(0,0,0,1); card:SetFrameLevel(20); card:Hide(); card.rows = {}
...
local function ShowRecipe(name, mats)
    ...
    for m in string.gfind(mats, "([^,]+)") do if card.rows[rowIdx] then local r = card.rows[rowIdx]; r:Show(); m = string.gsub(m, "^%s*(.-)%s*$", "%1"); local mN, mC = string.match(m, "(.+) x(%d+)"); if not mN then mN = m; mC = 1 end; local countInInv = GetItemCountByName(mN); local color = (countInInv >= tonumber(mC)) and "|cff00ff00" or "|cffaaaaaa"; r.text:SetText(color..m.." (Bag: "..countInInv..")|r"); r:SetScript("OnClick", function() cb:SetText(mN); cb:Show(); cb:SetFocus(); cb:HighlightText() end); rowIdx = rowIdx + 1 end end
    card:Show()
end
local cardClose = CreateFrame("Button", nil, card, "UIPanelCloseButton"); cardClose:SetPoint("TOPRIGHT", -2, -2)

-- MAIN CLOSE BUTTON
local mainClose = CreateFrame("Button", "TheCultScams_MainCloseButton", frame, "UIPanelCloseButton")
mainClose:SetPoint("TOPRIGHT", -5, -5)
mainClose:SetFrameLevel(10)
...
local function GetRow(idx) if not rows[idx] then local r = CreateFrame("Button", nil, content); r:SetWidth(260); r:SetHeight(16); r:SetPoint("TOPLEFT", 0, -(idx-1)*16); r:SetBackdrop({bgFile="Interface\\Buttons\\WHITE8X8"}); r:SetBackdropColor(0,0,0,0); r.text = r:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall"); r.text:SetPoint("LEFT", 5, 0); r:RegisterForClicks("LeftButtonUp", "RightButtonUp"); rows[idx] = r end; rows[idx]:Show(); return rows[idx] end
...
local chatBox = CreateFrame("EditBox", nil, frame, "InputBoxTemplate"); chatBox:SetWidth(200); chatBox:SetHeight(20); chatBox:SetPoint("BOTTOMLEFT", 15, 35); chatBox:SetAutoFocus(false); chatBox:SetScript("OnEnterPressed", function() TheCultScamsCore:SendGroupMessage(currentGroup, this:GetText()); this:SetText(""); this:ClearFocus() end); local footer = frame:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall"); footer:SetPoint("BOTTOM", 0, 10); footer:SetText("The Cult Scams v4.4 [MINT] TS: 10:25:42"); footer:SetTextColor(0.5, 0.5, 0.5)