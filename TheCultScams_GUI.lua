-- <The Cult Scams> Matte UI - v5.0 [MAROON] TS: 11:37:07
...
frame:SetBackdropColor(0.2, 0.05, 0.05, 0.95); frame:Hide()
...
local card = CreateFrame("Frame", "TheCultScams_RecipeCard", frame); card:SetWidth(280); card:SetHeight(180); card:SetPoint("CENTER", 0, 20); card:SetBackdrop({bgFile="Interface\\Tooltips\\UI-Tooltip-Background", edgeFile="Interface\\Tooltips\\UI-Tooltip-Border", edgeSize=12}); card:SetBackdropColor(0,0,0,1); card:SetFrameLevel(25); card:Hide(); card.rows = {}
for idx=1, 10 do local r = CreateFrame("Button", nil, card); r:SetWidth(260); r:SetHeight(16); r:SetPoint("TOPLEFT", 10, -30-(idx-1)*16); r.text = r:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall"); r.text:SetPoint("LEFT", 0, 0); r:RegisterForClicks("LeftButtonUp", "RightButtonUp"); card.rows[idx] = r end
...
local function ShowRecipe(name, mats)
    ...
    for m in string.gfind(mats, "([^,]+)") do if card.rows[rowIdx] then 
        local r = card.rows[rowIdx]; r:Show(); m = string.gsub(m, "^%s*(.-)%s*$", "%1")
        local _, _, mN, mC = string.find(m, "(.+) x(%d+)")
        if not mN then _, _, mN, mC = string.find(m, "(.+) %((%d+)%)") end
        if not mN then mN = m; mC = 1 end
        local countInInv = GetItemCountByName(mN); local color = (countInInv >= tonumber(mC)) and "|cff00ff00" or "|cffaaaaaa"
        r.text:SetText(color..m.." (Bag: "..countInInv..")|r")
        r:SetScript("OnClick", function() cb:SetText(mN); cb:Show(); cb:SetFocus(); cb:HighlightText() end)
        rowIdx = rowIdx + 1 
    end end
    card:Show()
end
...
local function Row_OnClick()
    if TheCultScamsDB and TheCultScamsDB.DebugMode then DEFAULT_CHAT_FRAME:AddMessage("|cffaaaaaa[SCAM Debug]|r Action: " .. (this.action or "NONE")) end
...
local function GetRow(idx)
    if not rows[idx] then
        ...
        r:SetFrameLevel(15); r:SetBackdrop({bgFile="Interface\\Buttons\\WHITE8X8"}); r:SetBackdropColor(1,1,1,0.02)
        r:SetHighlightTexture("Interface\\Buttons\\UI-PlusButton-Hilight", "ADD")
        r.text = r:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall"); r.text:SetPoint("LEFT", 5, 0)
...
function TheCultScams_UpdateGUI()
    if not TheCultScamsDB then return end; for _, r in pairs(rows) do r:Hide() end; if getglobal("TheCultScams_GroupInput") then getglobal("TheCultScams_GroupInput"):Hide() end; if getglobal("TheCultScams_GroupBtn") then getglobal("TheCultScams_GroupBtn"):Hide() end; card:Hide(); local rowIdx = 1
    ...
    elseif currentTab == "SEARCH" then local sBox = getglobal("TheCultScams_SearchBox"); if sBox then sBox:Show() end; scrollFrame:SetPoint("TOPLEFT", 15, -100); local q = string.lower(getglobal("TheCultScams_SearchBox"):GetText() or ""); for p, pData in pairs(TheCultScamsDB.Players) do for prof, recipes in pairs(pData) do for rName, data in pairs(recipes) do if q ~= "" and string.find(string.lower(rName), q, 1, true) then AddRow(data.link .. " (|cff00ff00" .. p .. "|r)", 1, 1, 1, "SHOW_RECIPE", rName, data.mats) end end end end
    elseif currentTab == "CHAT" then if getglobal("TheCultScams_GroupInput") then getglobal("TheCultScams_GroupInput"):Show() end; if getglobal("TheCultScams_GroupBtn") then getglobal("TheCultScams_GroupBtn"):Show() end; scrollFrame:SetPoint("TOPLEFT", 15, -100); AddRow("|cff00ffff[GROUP MANAGER]|r", 0, 1, 1); for gName in pairs(TheCultScamsDB.CustomGroups) do local sel = (gName == currentGroup) and "> " or "  "; AddRow(sel..gName, 1, 1, 1, "SELECT_GROUP", gName); if gName == currentGroup and gName ~= "General" then AddRow("  |cff00ff00+ Add Member|r", 0, 1, 0, "ADD_MEMBER"); for mem in pairs(TheCultScamsDB.CustomGroups[gName]) do AddRow("    |cffff8000- "..mem.."|r", 1, 0.5, 0, "REMOVE_MEMBER", mem) end end end; AddRow("--- Messages ("..currentGroup..") ---", 0.5, 0.5, 0.5); local msgs = TheCultScamsDB.Messages[currentGroup] or {}; for _, m in ipairs(msgs) do AddRow("["..m.sender.."]: "..m.text, 0.8, 0.6, 0.8, "WHISPER", m.sender) end end
    content:SetHeight(rowIdx * 16)
end
StaticPopupDialogs["SCAM_ADD_MEMBER"] = { text = "Add player to " .. currentGroup, button1 = "Add", button2 = "Cancel", hasEditBox = 1, OnAccept = function() local n = getglobal(this:GetParent():GetName().."EditBox"):GetText(); if n ~= "" then TheCultScamsDB.CustomGroups[currentGroup][n] = true; TheCultScams_UpdateGUI() end end, timeout = 0, whileDead = 1, hideOnEscape = 1 }; local groupInput = CreateFrame("EditBox", "TheCultScams_GroupInput", frame, "InputBoxTemplate"); groupInput:SetWidth(120); groupInput:SetHeight(20); groupInput:SetPoint("TOPLEFT", 15, -70); groupInput:SetAutoFocus(false); groupInput:Hide(); local groupBtn = CreateFrame("Button", "TheCultScams_GroupBtn", frame, "UIPanelButtonTemplate"); groupBtn:SetWidth(60); groupBtn:SetHeight(20); groupBtn:SetPoint("LEFT", groupInput, "RIGHT", 5, 0); groupBtn:SetText("New"); groupBtn:Hide(); groupBtn:SetScript("OnClick", function() local n = groupInput:GetText(); if n ~= "" then TheCultScamsDB.CustomGroups[n] = {}; TheCultScams_UpdateGUI() end end); frame:SetScript("OnShow", function() TheCultScams_UpdateGUI() end); frame:SetScript("OnDragStart", function() this:StartMoving() end); frame:SetScript("OnDragStop", function() this:StopMovingOrSizing() end); local resizer = CreateFrame("Button", nil, frame); resizer:SetWidth(10); resizer:SetHeight(10); resizer:SetPoint("BOTTOMRIGHT", 0, 0); resizer:SetNormalTexture("Interface\\ChatFrame\\UI-ChatIM-SizeGrabber-Up"); resizer:SetScript("OnMouseDown", function() this:GetParent():StartSizing("BOTTOMRIGHT") end); resizer:SetScript("OnMouseUp", function() this:GetParent():StopMovingOrSizing(); TheCultScams_UpdateGUI() end); local mainClose = CreateFrame("Button", "TheCultScams_MainCloseButton", frame, "UIPanelCloseButton"); mainClose:SetPoint("TOPRIGHT", -5, -5); mainClose:SetFrameLevel(30); mainClose:SetScript("OnClick", function() frame:Hide() end)
...
local chatBox = CreateFrame("EditBox", nil, frame, "InputBoxTemplate"); chatBox:SetWidth(200); chatBox:SetHeight(20); chatBox:SetPoint("BOTTOMLEFT", 15, 35); chatBox:SetAutoFocus(false); chatBox:SetScript("OnEnterPressed", function() TheCultScamsCore:SendGroupMessage(currentGroup, this:GetText()); this:SetText(""); this:ClearFocus() end); local footer = frame:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall"); footer:SetPoint("BOTTOM", 0, 10); footer:SetText("The Cult Scams v5.0 [MAROON] TS: 11:37:07"); footer:SetTextColor(0.5, 0.5, 0.5); local cardClose = CreateFrame("Button", nil, card, "UIPanelCloseButton"); cardClose:SetPoint("TOPRIGHT", -2, -2)